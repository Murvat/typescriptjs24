...
// Task 7.
// Напишите класс User_07, который содержит два свойства: _age, число private равное по умолчанию 0 и _adult private, по умолчанию false.  Напишите сеттер age, который устанавливает число age, если переданный аргумент больше нуля и меньше 130. В противном случае ставит 0. 
//Также в сеттере реализуйте установку _adult в зависимости от возраста.Если возраст(_age) больше 16 mо _adult true.Во всех остальных случаях - false.Напишите геттеры age, adult.Реализуйте установку age через конструктор с помощью сеттера.
...
class User_07 {
    ...
    set age(num: number) {
        num < 130 && num > 0 ? this._age = num : this._age = 0;
        num > 16 ? this._adult = true : this._adult = false;
    };
    get age(): number {
        return this._age;
    };
...
// Task 15
// В классе SignupForm мы допустили ошибку - делать toLowerCase паролю не самая лучшая идея. Давайте сделаем класс SignupForm_15,
//  который наследуется от SignupForm и исправим конструктор, и изменим конструктор - в нем мы будем для email применять prepareString,
//     а для password и passwordRepeat будем делать только операцию trim.
