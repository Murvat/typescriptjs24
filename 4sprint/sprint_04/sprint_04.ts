...
function f05(): string {
    let ranLetter: string;
    let randomNum: number = Math.floor(Math.random() * s5.length);
    console.log(randomNum)
    ranLetter = s5[randomNum];
    return ranLetter;
}
...
function f07(): void {
    let randomNum: number = Math.floor(Math.random() * 20);
    let res: number;
    ...
    document.querySelector('.out-7').textContent = String(res)
}
...
function f12(pass: string): boolean {
    if (pass.length <= 8 || pass.toLocaleLowerCase() != pass) {
        return false
    } else {
        return true;
    }
...
function f17(s: string): string | boolean {
    let elemP = document.querySelector(s).textContent;
    if (elemP) {
        return elemP.trim();
    } else {
        return false;
...
function f20(n1?: number, n2?: number): number {
    if (n1 !== undefined && n2 !== undefined) {
        ...
        if (n1 > n2) {
            randomNum = Math.floor(Math.random() * (n1 - n2));
            for (let i = n2; i < n1; i++) {
                if (randomNum === i - n2) {
                    res = i;
                }
            ...
            randomNum = Math.floor(Math.random() * (n2 - n1));
            for (let i = n1; i < n2; i++) {
                if (randomNum === i - n1) {
                    res = i;
                }
