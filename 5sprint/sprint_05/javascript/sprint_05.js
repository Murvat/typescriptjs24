...
const f02 = (str) => str.trim().toLocaleLowerCase();
...
const f03 = (str = '') => str.trim().toLocaleLowerCase();
...
const f05 = (className) => parseInt(document.querySelector(className).value);
...
const f08 = (a, b) => {
    let maxNum = Math.max(a, b); //большее число;
    let minNum = Math.min(a, b); //меньшее число;
    let randomNum = Math.floor(Math.random() * (maxNum - minNum));
    let res; //число в диапазоне;
