...
class Paragraph_05 {
    ...
    render() {
        ...
        let firstLetter = this.text.charAt(0).toUpperCase();
...
class List_08 {
    ...
    ;
    render() {
        let ulElem = document.createElement(this.type);
        for (let item of this.data) {
            let liElem = document.createElement(this.type);
            liElem.textContent = item;
            ulElem.appendChild(liElem);
        }
        return ulElem;
...
class Palindrome {
    str;
    isPalindrome;
    constructor(str, isPalindrome = false) {
        this.str = str;
